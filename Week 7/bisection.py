...
def bisection_method(f, a, b, tol, iterations=0):
    # Add a horizontal line at y = 0 if it is the first iteration
    if iterations == 0:
    ...
    # If f(a) and f(m) have different signs, recursively call bisection_method with m as the new a
        ...
        color = plt.colormaps.get_cmap('autumn_r')(iterations / 10)  # Get inverse autumn color based on iteration
        plt.plot(m, f(m), 'o', color=color)  # Plot the current root with inverse autumn color
